...
pub fn register(io: std.Io, gpa: std.mem.Allocator, lua: *Lua) void {
    ...
    inline for (@typeInfo(@This()).@"struct".decls) |decl| {
        ...
        if (@TypeOf(d) != ZigFn) continue;

        lua.register("_branch_" ++ decl.name, zlua.wrap(d));
    }
}
...
///@param activate fun()
///@return branch.Item
pub fn new_item(lua: *zlua.Lua) i32 {
    if (lua.getTop() != 3) {
...
///@param ... branch.Item
///@return branch.Item
pub fn new_menu(lua: *zlua.Lua) i32 {
    if (lua.getTop() < 3) {
...
pub fn new_field(lua: *zlua.Lua) i32 {
    if (lua.getTop() != 5) {
...
pub fn new_form(lua: *zlua.Lua) i32 {
    if (lua.getTop() < 4) {
...
///@param key? string # Key to activate item
///@return branch.Item
pub fn new_none(lua: *zlua.Lua) i32 {
    if (lua.getTop() != 2) {
...
pub fn open_url(lua: *zlua.Lua) i32 {
